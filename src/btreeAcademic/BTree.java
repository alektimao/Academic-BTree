...
//    }
    ...
    // Remover
//    public BTreeNode FusaoFilhoPai(BTreeNode pai)//fusao filho com seu pai
//    {
...
//    }
    public void remove(int info) {
        boolean certo = false;
        BTreeNode Nofolha = null, pai = null, aux1 = null;
        int i = 0, pos = 0, aux = 0;

        if (raiz == null) {
            System.out.println("A árvore sem elementos");
        } else {
            Nofolha = navegaAteFolha(info);
            if (Nofolha != null)// validaçao do no!
            {
                if (Nofolha.isFolha())//Valida para saber se é uma folha
                {
                    if (Nofolha == raiz) // A folha é a raiz da arvore
                    {
                        Nofolha.removeInfo(Nofolha.procuraPosicao(info));
                        certo = true;
                    } else {
                        pai = localizaPai(Nofolha, info);
                        Nofolha.removeInfo(Nofolha.procuraPosicao(info));

                        if (Nofolha.getTl() < N)// Caso o TL fique menor que o N tem que emprestar.
                        {
                            aux1 = Nofolha;
                            i = aux1.procuraPosicao(info);
                            if (i == 0) {
                                // ok = PegarDireita(info, i, folha, pai);
                            } else if (i > 0 && i < pai.getTl())//meio
                            {
                                // ok = pegaresquerda(info, i - 1, folha, pai);
                                if (!certo)//Não conseguiu
                                {
                                    //  ok = PegarDireita(info, i, folha, pai);
                                }
                            } else {
                            ...
                            if (!certo) {
                                System.out.println("fazer fusao");
                                if (i == 0) {
                                    fusao(i, Nofolha, pai);
                                    certo = true;
                                } else {
                                    fusao(i - 1, Nofolha, pai);
                                    certo = true;
                                }
                            }
                        } else {
...
//                }
                ...
                System.out.println("nao existe info.");
            }
        }
    }
